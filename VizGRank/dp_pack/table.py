...
class Table(object):
        ...
        if type(minTime) != type(datetime.date(1995,10,11)) and minTime.year == maxTime.year and minTime.month == maxTime.month and minTime.day == maxTime.day:
            ...
            if minHour == maxHour:
                if minMinute == maxMinute:
                    # year, month, day, hour, minute of max and min is the same, so
                    interval = 'SECOND'
                    ...
                    # year, month, day, hour of max and min is the same, but minute is different, so
                    interval = 'MINUTE'
                ...
                # year, month, day of max and min is the same, but hour is different, so
                interval = 'HOUR'
            ...
            if minYear == maxYear:
                if minMonth == maxMonth:
                    # year, month of max and min is the same, but hour is different, so
                    interval = 'DAY'
                    for i in range(minDay, maxDay + 1):
                        bins.append([str(i)+'th',datetime.date(minYear, minMonth, i),datetime.date(minYear, minMonth, i), 0])
                else:
                    # year of max and min is the same, but hour is different, so
                    interval = 'MONTH'
                ...
                # year of max and min is different, so
    ...
    def generateViews(self):
        ...
        """
        with monitor('Feature extraction', level=3):
        ...
        with monitor('View combination', level=3):
            #generate 2D views
...
# The following functions deal with different types of data and return new_table
            ...
            for i in range(begin,end):
                ...
                for j in range(self.column_num):
                    if self.types[j] == Type.numerical:
                        if isinstance(self.D[i][j], int) or isinstance(self.D[i][j], float) or isinstance(self.D[i][j], str):#self.D[i][j] != '' and :
                            d[self.D[i][column_id]][sum_column] += float(self.D[i][j]) if self.D[i][j] else 0
                        sum_column += 2
            for k in d:
    ...
    def dealWithIntervalBin(self,column_id,begin,end,get_head,get_data):
        ...
        bins = list(map(lambda x: [x[0], x[1], x[2], 0], bins))
                    ...
                    if bins[j][1] <= date <= bins[j][2]:
                        ...
                        for k in range(self.column_num):
                            if self.types[k] == Type.numerical:
                                if isinstance(self.D[i][k], int) or isinstance(self.D[i][k], float) or isinstance(self.D[i][k], str):
                                    new_table.D[j][sum_column] += float(self.D[i][k]) if self.D[i][k] else 0
                                sum_column += 2
                        break
    ...
    def dealWithWeekBin(self,column_id,begin,end,get_head,get_data):
        ...
        if get_data:
            ...
            for i in range(begin,end):
                ...
                for j in range(self.column_num):
                    if self.types[j] == Type.numerical:
                        if isinstance(self.D[i][j], int) or isinstance(self.D[i][j], float) or isinstance(self.D[i][j], str):
                            new_table.D[weekday][sum_column] += float(self.D[i][j]) if self.D[i][j] else 0
                        sum_column += 2
            for i in range(7):
                for j in range(2,num + 2,2):
                    if new_table.D[i][1]:
    ...
    def dealWithTable(self):
        ...
        with monitor('Transformation enumeration', level=3):
            for i in range(self.column_num):
